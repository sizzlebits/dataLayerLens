...
describe('generateUUID', () => {
  ...
  describe('fallback implementation', () => {
    ...
    it('generates valid UUID v4 format when crypto.randomUUID is unavailable', () => {
      const uuid = generateUUID();
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
      expect(uuid).toMatch(uuidRegex);
    });

    it('generates unique UUIDs with fallback', () => {
      const uuids = new Set<string>();
      for (let i = 0; i < 100; i++) {
        uuids.add(generateUUID());
      }
    ...
    it('fallback version number is 4', () => {
      const uuid = generateUUID();
      expect(uuid[14]).toBe('4');
    });

    it('fallback variant bits are correct (8, 9, a, or b)', () => {
      for (let i = 0; i < 10; i++) {
        const uuid = generateUUID();
        expect(['8', '9', 'a', 'b']).toContain(uuid[19].toLowerCase());
      }
